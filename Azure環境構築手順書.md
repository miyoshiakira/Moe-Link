...
| Azure OpenAI Service | `aoai-moelink` | GPT-4o Realtime API（音声通話） |
...
> **リージョンの注意点**
> `gpt-4o-realtime-preview` は **East US 2** または **Sweden Central** でのみ利用可能（2025年3月時点）。
> Japan East では現時点でデプロイできないため、East US 2 を推奨。

### 2-2. モデルのデプロイ

1. 作成した Azure OpenAI リソースを開く
2. 左メニューから **「Azure OpenAI Studio に移動」** をクリック（または `https://oai.azure.com` へ直接アクセス）
3. 左メニューの **「デプロイ」** を選択
4. **「＋ モデルのデプロイ」→「基本モデルをデプロイする」** をクリック
5. モデル一覧から **`gpt-4o-realtime-preview`** を選択して **「確認」**
6. 以下を設定して **「デプロイ」**

| 項目 | 値 |
|---|---|
| デプロイ名 | `gpt-4o-realtime-preview`（`.env` の `AZURE_OPENAI_DEPLOYMENT` に使用） |
| モデルバージョン | 最新版を選択 |
| デプロイの種類 | `Standard` |
...
| `.env` の変数名 | 取得場所 |
|---|---|
| `AZURE_OPENAI_ENDPOINT` | 「エンドポイント」欄（例：`https://aoai-moelink.openai.azure.com`） |
...
Azure OpenAI Studio（`https://oai.azure.com`）の **「リアルタイム音声」** メニューから
デプロイした `gpt-4o-realtime-preview` に話しかけて応答が返れば正常。
...
開発・学習用途での概算（2025年3月時点、実際の利用量により変動）。

| リソース | 価格レベル | 月額目安 |
|---|---|---|
| Azure OpenAI GPT-4o Realtime | Standard / トークン従量 | 数百〜数千円（通話量による） |
...
**「gpt-4o-realtime-preview が一覧に表示されない」**
